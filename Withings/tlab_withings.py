...
if __name__ == "__main__":
    ...
    try:
        while(True):
            for i in range (0, len(user_ids)):
                ...
                for measure in measures:
                    ...
                    for group in measure:
                        for result in group.measures:  
                            date = group.date.replace(second=0).replace(minute=0).replace(hour=0)                                                  
                            if(result["type"] == 11): #Average Heartrate
                                avg_heartrate[date] = normalize_value(result['value'], result['unit'])
                            elif(result["type"] == 71): #Body Temperature
                                bodytemp[date] = normalize_value(result['value'], result['unit'])
                            elif(result["type"] == 10): #Systolic BP
                                bp[date] = []
                                bp[date].append(normalize_value(result['value'], result['unit']))
                ...
                with codecs.open(str(user_id) + "_sleep_summary.csv", "w", "utf-8") as f:
                    ...
                    for record in sleep_summary['series']:
                        this_measurement = parse(record['date']).date()
            ...
            logging.debug("Sleeping for " + str(sleep_duration) + " seconds until next round.")            
            time.sleep(sleep_duration) #sleep for 10 seconds and query stats again
